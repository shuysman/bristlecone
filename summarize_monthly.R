...
soil_plt_post_2010 <-summary_tbl %>%
    ggplot(mapping = aes(label = site)) +
    geom_point(mapping = aes(x = mean_t_pre_2010, y = mean_SOIL_post_2010, color = loc)) +
...
# 3d plot example
plot_ly(x = summary_tbl$mean_t_post_2010, y = summary_tbl$mean_P_post_2010, z = summary_tbl$mean_SOIL_post_2010,  type="scatter3d", mode="markers", color = summary_tbl$loc)
