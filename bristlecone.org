...
#+EXPORT_FILE_NAME: exports/index.html
...
***** AET
****** TODO Hamon
#+begin_src R :session :file all_variables_hamon_daymet_swb.png :results graphics file :eval never-export :exports both
  summary_tbl_by_year_hamon %>%
    mutate(loc = fct_relevel(find_loc(site), c("panamint", "wah_wah", "silver_peak", "rawhide", "other"))) %>%
    ggplot() +
    geom_line(mapping = aes(x = year, y = AET), color = loc) +
    scale_color_manual(name = 'Location', values = loc_colors)
#+end_src
****** TODO Penman
***** CWD
